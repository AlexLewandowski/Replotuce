...
function get_config_data(results_dir)
    ...
    config_dir = joinpath(results_dir, "settings")
    ...
    config_file = joinpath(config_dir, conf[1])

    return config_file, data_dir
end

function get_plots(; results_dir = "_results/", primary_metric_key = "rollout_returns", top_n = 3, profiler = [[[]]], AUC = true)
    sweep_dict, auc_score_dict, best_score_dict, key_list, metric_keys = get_dicts(results_dir = results_dir, primary_metric_key = primary_metric_key)
    if AUC
...
function get_dicts(; results_dir = "_results/", primary_metric_key = "rollout_returns")
    ...
    if isfile(dict_path)
        ...
        auc_score_dict = gen_scores(sweep_dict = sweep_dict, primary_metric_key = primary_metric_key, AUC = true)
        best_score_dict = gen_scores(sweep_dict = sweep_dict, primary_metric_key = primary_metric_key, AUC = false)
